...
class structure_encoder(nn.Module):
    ...
    def forward(self, h_V, edge_index, h_E, seq=None, batch=None):
        ...
        with autocast():  # Automatically handles FP16 and FP32 operations
            h_V = self.W_v(h_V)  # tuple (s,V) of node embeddings
            h_E = self.W_e(h_E)  # in gearnet added h_edge = self.rbf((pos_out - pos_in).norm(dim=-1)), vec_edge why?
            for layer in self.layers:
                h_V = layer(h_V, edge_index, h_E)
            out = self.W_out(
                h_V)  # in ESM-IF https://github.com/facebookresearch/esm/blob/main/esm/inverse_folding/gvp_encoder.py they don't have this out!
