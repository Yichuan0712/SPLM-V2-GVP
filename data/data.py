...
def custom_collate(one_batch):
    ...
    plddt_avg_scores = torch.tensor([x.mean().item() for x in plddt_scores])
...
def custom_collate_mask(one_batch):
    ...
    batched_data = {'graph': torch_geometric_batch, 'mask_graph': torch_geometric_batch_mask, 'seq': raw_seqs,
                    'plddt': plddt_scores, 'pid': pids, 'coordinates': coordinates,
                    'coordinates_mask': coordinates_mask, "masks": masks, "plddt_avg": plddt_avg_scores}
...
class ProteinGraphDataset(Dataset):
    ...
    def __init__(self, data_path, max_length=512,
                 ...
                 batch_size=0,
                 saved_batch_index=0,
                 shared_first_epoch=None):
        super(ProteinGraphDataset, self).__init__()

        # self.h5_samples = glob.glob(os.path.join(data_path, '*.h5'))
        self.h5_paths = glob.glob(os.path.join(data_path, '*.h5'))  # Store paths, not file handles
        self.top_k = top_k
        ...
        # only work for training, resume from a saved_batch_index not from beginning!
        self.batch_size = batch_size
        self.saved_batch_index = saved_batch_index
        self.first_epoch = shared_first_epoch  # ✅ Assign the shared variable
        if shared_first_epoch is not None:
            print(
                f"[DEBUG] Dataset initialized in process {mp.current_process().name} - first_epoch = {self.first_epoch.value}")
    ...
    def __len__(self):
        """Ensure DataLoader only requests valid indices during the first epoch."""
        total_samples = len(self.h5_paths)
        if self.first_epoch is None:
            return total_samples

        if self.first_epoch.value:
            remaining_samples = total_samples - (self.saved_batch_index * self.batch_size)
            # print(f"[DEBUG] dataset len is {max(remaining_samples, 0)}")
            return max(remaining_samples, 0)  # Prevent negative values
        else:
            # print(f"[DEBUG] dataset len is {total_samples}")
            return total_samples  # Use full dataset after the first epoch

    def __getitem__(self, i):
        if self.first_epoch is not None:
            if self.first_epoch.value:
                actual_index = (self.saved_batch_index * self.batch_size + i)  # % len(self.h5_paths)
            else:
                actual_index = i  # Normal behavior after first epoch
        else:
            actual_index = i

        sample_path = self.h5_paths[actual_index]
        ...
        # print(f"coords type: {type(coord)}")
...
class ProteinGraphDataset_mask(ProteinGraphDataset):
    # ✅ Define shared variables at the class level
    _manager = None
    _global_first_epoch = None
    ...
    def __init__(self, data_path, max_length,
        ...
        self.first_epoch = shared_first_epoch  # ✅ Assign the shared variable
        # if shared_first_epoch is not None:
        #   print(f"[DEBUG] Dataset initialized in process {mp.current_process().name} - first_epoch = {self.first_epoch.value}")

        """
        # ✅ Initialize shared variable if it's not already initialized
        if ProteinGraphDataset_mask._manager is None:
            ProteinGraphDataset_mask._manager = mp.Manager()
            ProteinGraphDataset_mask._global_first_epoch = ProteinGraphDataset_mask._manager.Value('b', True)  # 'b' = boolean
        
        print(f"[DEBUG mask] Dataset initialized in process {mp.current_process().name} - first_epoch = {ProteinGraphDataset_mask._global_first_epoch.value}")
        """
    ...
    def mask_coords(self, coords):
        ...
        probability_matrix = torch.full((coords.shape[0], coords.shape[1], 1), self.mlm_probability,
    ...
    def __getitem__(self, i):
        # print(f"[DEBUG mask] in getitem dataset object ID: {id(self)}")
        # print(f"[DEBUG mask] in getitem first_epoch {self.first_epoch}")
        if self.first_epoch is not None:
            if self.first_epoch.value:
                actual_index = (self.saved_batch_index * self.batch_size + i)  # % len(self.h5_paths)
                # print("[DEBUG mask] actual_index="+str(actual_index))
            else:
                # print("[DEBUG mask] i="+str(i))
                actual_index = i  # Normal behavior after first epoch
        else:
            # print("[DEBUG mask validation] i="+str(i))
            actual_index = i

        # print("[DEBUG mask] len(self.h5_paths)="+str(len(self.h5_paths)))
        sample_path = self.h5_paths[actual_index]
        sequence, coord, plddt_scores = load_h5_file(sample_path)
        coord = coord[:self.max_length]
        basename = os.path.basename(sample_path)
...
def worker_init_fn(worker_id):
    ...
    dataset = worker_info.dataset  # ✅ Get dataset instance assigned to this worker
    # print(f"[DEBUG] Worker {worker_id} initialized with dataset ID: {id(dataset)}")


def prepare_dataloaders(logging, accelerator, configs, saved_batch_index, shared_first_epoch):
    ...
    train_dataset = ProteinGraphDataset(configs.train_settings.data_path,
                                        ...
                                        saved_batch_index=saved_batch_index,
                                        shared_first_epoch=shared_first_epoch,
                                        )
    ...
    train_loader = DataLoader(train_dataset, batch_size=configs.train_settings.batch_size,
                              ...
                              multiprocessing_context='spawn' if configs.train_settings.num_workers > 1 else None,
                              pin_memory=True,
    ...
    val_loader = DataLoader(val_dataset, batch_size=configs.valid_settings.batch_size,
                            ...
                            shuffle=False,
                            multiprocessing_context='spawn' if configs.valid_settings.num_workers > 1 else None,
                            pin_memory=True,
                            collate_fn=custom_collate,
                            persistent_workers=True)
...
def prepare_mask_dataloaders(logging, accelerator, configs, saved_batch_index, shared_first_epoch):
    if accelerator.is_main_process:
        logging.info(f"train directory: {configs.train_settings.data_path}")
        logging.info(f"valid directory: {configs.valid_settings.data_path}")
    ...
    # Prevents workers from closing/reopening HDF5 file)
    val_loader = DataLoader(val_dataset, batch_size=configs.valid_settings.batch_size,
                            num_workers=configs.valid_settings.num_workers,
                            shuffle=False,
                            multiprocessing_context='spawn' if configs.valid_settings.num_workers > 1 else None,
