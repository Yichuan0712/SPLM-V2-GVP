...
def preprocess_file(file_path, max_len, save_path, dictn, report_dict, similarity_threshold):
...
def main():
    ...
    parser.add_argument('--similarity_threshold', default=1.0, type=float,
                        help='Threshold for considering two sequences as similar. If not 1.0, only the unique sequences with the most resolved Cα atoms will be retrained for each PDB file')

    args = parser.parse_args()

    data_path = [os.path.join(args.data, path) for path in os.listdir(args.data) if
                 os.path.isfile(os.path.join(args.data, path))]  # and path.endswith('pdb')]
    ...
    with Manager() as manager:
        ...
        with ProcessPoolExecutor(max_workers=args.max_workers) as executor:
            futures = {executor.submit(preprocess_file, file_path, args.max_len, args.save_path, dictn, report_dict,
                                       similarity_threshold): file_path for file_path in data_path}
...
# python3 preprocess_pdb.py --data /cluster/pixstor/xudong-lab/duolin/CATH/CATH_4_3_0_non-rep_pdbs/ --save_path /cluster/pixstor/xudong-lab/duolin/CATH/CATH_4_3_0_non-rep_gvp_v1/ --max_workers 4
