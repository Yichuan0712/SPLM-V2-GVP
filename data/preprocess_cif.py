...
def main():
    parser = argparse.ArgumentParser(description='Processing CIF files.')
    parser.add_argument('--data', default='./test_data', help='Path to CIF files.')
    ...
    data_path = [os.path.join(args.data, path) for path in os.listdir(args.data) if
                 os.path.isfile(os.path.join(args.data, path)) and path.endswith('cif')]
